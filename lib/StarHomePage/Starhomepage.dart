...
import 'package:star_event/Responsive/StarDetailsScreenList.dart';
import 'package:star_event/User/NonInterestWidget.dart';
import 'package:step_progress_indicator/step_progress_indicator.dart';

import 'StarSearchBar.dart';
...
const colorizeTextStyle = TextStyle(fontSize: 20, fontFamily: 'poppinsbold');
...
class StarHomePage extends StatefulWidget {
  StarHomePage();
  @override
  _StarHomePageState createState() => _StarHomePageState();
}
...
class _StarHomePageState extends State<StarHomePage> {
  ...
  Future<List> getData() async {
    ...
    var url =
        Uri.parse("http://5howapp.com/StarLoginAndRegister/getStarsSL.php");
    print(storage.getItem("Interest"));
    var response = await client.post(url, headers: {
      "Accept": "application/json",
      "Access-Control_Allow_Origin": "*"
    }, body: {
      "staremail": storage.getItem("email"),
      "interest": "Football",
    });
    print(storage.getItem('interest'));
    print(response.body);
    ...
    storage.setItem("starhomedata", dataRecieved);
    return storage.getItem("starhomedata");
    storage.setItem("userhomedata", dataRecieved);
    return storage.getItem("userhomedata");
  }
...
class _UserHomeScreenState extends State<UserHomeScreen> {
  ...
  Future<List> getupcommingsession() async {
    final LocalStorage storage = new LocalStorage('Star');
    var client = http.Client();

    var url = Uri.parse(
        "http://5howapp.com/StarLoginAndRegister/Zoom/Zoom/starupcomingevents.php");
    var response = await client.post(url, headers: {
      "Accept": "application/json",
      "Access-Control_Allow_Origin": "*"
    }, body: {
    ...
    print("Upcomming event......${dataRecieved[0]['topic']}");

    return uploadupcomingsession(
        dataRecieved[0]['topic'],
        dataRecieved[0]['participantcount'],
        dataRecieved[0]['totalparticipantcount'],
        dataRecieved[0]['Month']);
  }

  uploadupcomingsession(String topic, String participantcount,
      String totalparticipantcount, String Month) async {
    var client = http.Client();
    var url1 = Uri.parse(
        "http://5howapp.com/StarLoginAndRegister/uploadUpcomingSession.php");
    var response1 = await client.post(url1, headers: {
      "Accept": "application/json",
      "Access-Control_Allow_Origin": "*"
    }, body: {
      "email": storage.getItem("email"),
      "upcomingsession": topic,
      "upcommingsessiondate": Month,
      "upcommingparticipantcount": participantcount,
      "upcommingtotalparticipantcount": totalparticipantcount
    });
    var dataRecieved1 = jsonDecode(response1.body);
  }
        ...
        child: FutureBuilder<Username>(
          ...
          builder: (context, snapshot) {
            if (snapshot.hasData) {
              ...
              storage.setItem("username", snapshot.data?.data.username);
              storage.setItem("profileimage", snapshot.data?.data.image);
              storage.setItem("PaymentKey", snapshot.data?.data.PaymentKey);
              storage.setItem("secret", snapshot.data?.data.secret);
              storage.setItem("profileimage", snapshot.data?.data.image);
              return Stack(
                children: <Widget>[
                  SafeArea(
                            ...
                            child: Container(
                              child: GridView.builder(
                                  ...
                                  itemBuilder: (context, index) {
                                    String interest = list[index]['interest'];
                                    return Padding(
                                      ...
                                      child: Stack(
                                        children: [
                                          ...
                                          list[index]['upcomingsession'] == null
                                              ? Padding(
                                                  padding:
                                          ...
                                          Padding(
                                            ...
                                            child: Stack(
                                              children: [
                                                ...
                                                list[index]['upcommingsessiondate'] ==
                                                        null
                                                    ? Center(
                                                        child: Padding(
                                                          padding:
                                                              const EdgeInsets
                                                                  ...
                                                                  right: 35),
                                                          child: Text(
                                                            '${list[index]['upcommingsessiondate']} -  Seat ${list[index]['upcommingparticipantcount']}/${list[index]['upcommingtotalparticipantcount']}',
                                                            style: TextStyle(
                                                              fontFamily:
                                                                  'Poppinsregular',
                                                              fontSize: 8,
                                              ...
                                              onTap: () {
                                                try {
                                                  Navigator.push(
                                                      context,
                                                      MaterialPageRoute(
                                                          builder: (context) => StarDetailsScreenList(
                                                              Name: list[index]
                                                                  ['username'],
                                                              Interest: list[
                                                                      index]
