...
import 'package:star_event/Responsive/StarEventBookingDetails.dart';
...
class UpcomingEvent extends StatefulWidget {
  ...
  String status;
  UpcomingEvent(
      {required this.email, required this.interest, required this.status});
  @override
  _UpcomingEventState createState() =>
      _UpcomingEventState(email, interest, status);
}
...
class _UpcomingEventState extends State<UpcomingEvent> {
  ...
  String interest;
  String status;
  _UpcomingEventState(this.email, this.interest, this.status);
  late List result;
        ...
        builder: (context, snapshot) {
          ...
          return snapshot.hasData
              ? new StarEventCalender1(
                  list: snapshot.data!, interest: interest, status: status)
              : new Center(
...
class StarEventCalender1 extends StatefulWidget {
  ...
  String status;
  StarEventCalender1(
      {required this.list, required this.interest, required this.status});
  @override
  _StarEventCalender1State createState() =>
      _StarEventCalender1State(list, interest, status);
}
...
class _StarEventCalender1State extends State<StarEventCalender1> {
  ...
  String interest;
  String status;
  _StarEventCalender1State(this.list, this.interest, this.status);
    ...
    return Padding(
      ...
      child: Container(
        ...
        child: ListView.builder(
            ...
            itemCount: list.length == 0 ? 0 : 1,
            itemBuilder: (BuildContext context, int index) {
              return Center(
                child: FlatButton(
                  onPressed: () {
                    if (status == list[index]['status']) {
                    } else if (int.parse(list[index]['startday']) <
                        DateTime.now().day) {
                    } else if (int.parse(list[index]['startmonth']) <
                        DateTime.now().month) {
                    } else if (int.parse(list[index]['startyear']) <
                        DateTime.now().year) {
                    } else {
                      Navigator.push(
                          context,
                          MaterialPageRoute(
                              builder: (context) => StarEventBookingDetails(
                                    name: list[index]['starname'],
                                    EventDate: list[index]['startdate'],
                                    duration: list[index]['duration'],
                                    email: list[index]['email'],
                                    password: list[index]['password'],
                                    link: list[index]['link'],
                                    image: list[index]['image'],
                                    Event: list[index]['topic'],
                                    starttime: list[index]['starttime'],
                                    price: list[index]['price'],
                                    participanttype: list[index]
                                        ['participanttype'],
                                    participantcount: int.parse(
                                        list[index]['participantcount']),
                                    profileimage: list[index]['profileimage'],
                                    description: list[index]['description'],
                                    eventId: list[index]['event_id'],
                                    status: list[index]['status'],
                                    starname: list[index]['starname'],
                                    interest: interest,
                                    totalparticipantcount: int.parse(
                                        list[index]['totalparticipantcount']),
                                    startday: list[index]['startday'],
                                    startmonth: list[index]['startmonth'],
                                    startyear: list[index]['startyear'],
                                  )));
                    }
                  },
                  child: Center(
